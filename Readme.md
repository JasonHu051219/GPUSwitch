...
**GPUSwitch** 是一款为 Windows 10/11 用户设计的轻量级显卡首选项管理工具。它通过“进程监听 + 动态重启”的技术方案，解决了系统自动分配显卡不准确、手动切换步骤繁琐的痛点。

---

## 🌟 程序简介

在 Windows 10/11 中，即便设置了“高性能”模式，部分程序仍可能调用集显。**GPUSwitch** 提供了一个全局管理界面，允许你一键管理所有应用的显卡偏好，并支持在程序启动时弹窗询问，确保你的每一帧性能都用在刀刃上。
...
3. **冷却**：执行 2 秒的阻塞式等待 (`time.sleep`)，确保操作系统彻底释放文件锁和驱动句柄。
